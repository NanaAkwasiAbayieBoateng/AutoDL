...
def build_tf_extension(build_ext, options):
    ...
    tensorflow_ops_lib.define_macros = options['MACROS']
    tensorflow_ops_lib.include_dirs = options['INCLUDES']
    tensorflow_ops_lib.sources = options['SOURCES'] + ['shouter/tensorflow/mpi_ops.cc']
    tensorflow_ops_lib.extra_compile_args = options['COMPILE_FLAGS'] + tf_compile_flags
    tensorflow_ops_lib.extra_link_args = options['LINK_FLAGS'] + tf_link_flags
    tensorflow_ops_lib.library_dirs = options['LIBRARY_DIRS']
    tensorflow_ops_lib.libraries = options['LIBRARIES']

    build_ext.build_extension(tensorflow_ops_lib)
...
setup(name='shouter',
      ...
      description='Distributed commication framework for TensorFlow.',
      author='Tanguofu.',
      long_description=textwrap.dedent('''\
          shouter is a distributed commication framework for TensorFlow. 
          The goal of shouter is to easy used all of bandthwith
          fast and easy to use.'''),
      url='https://github.com/tanguofu/AutoDL',
      classifiers=[
